...
const PDATA=[
  ...
  {num:'03',sub:'Arts & Culture Block',name:'Malviya',glow:'#ffe066',rgb:'255,224,102',base:38,max:150,ring:true,desc:'Fine Arts, Performing Arts, and a grand open-air amphitheatre for cultural events. Home of Nescafe — the beloved student café.',feats:['Amphitheatre','Art Studios','Nescafe Café']},
  {num:'04',sub:'Policy & Law Block',name:'Chanakya',glow:'#69f0ae',rgb:'105,240,174',base:30,max:140,ring:false,desc:'Home to the School of Law & Governance — moot courts, seminar rooms, and a specialised legal library. Features Madhuvan Canteen.',feats:['Moot Court','Legal Library','Madhuvan Canteen']},
  ...
  {num:'06',sub:'Student Residence',name:'Boys Hostel',glow:'#4fc3f7',rgb:'79,195,247',base:32,max:140,ring:false,desc:'Spacious and secure residential facility for male students — with reading rooms, common halls, Wi-Fi and 24-hour security.',feats:['Reading Room','Common Hall','Wi-Fi & Security']},
  {num:'07',sub:'Student Residence',name:'Girls Hostel',glow:'#f48fb1',rgb:'244,143,177',base:32,max:140,ring:false,desc:'Safe and comfortable residential facility for female students — with dining, recreation, wellness support and round-the-clock security.',feats:['Dining Facility','Recreation Room','24×7 Security']},
  {num:'08',sub:'Sports & Wellness',name:'Milkha Singh Sports Complex',glow:'#a5d6a7',rgb:'165,214,167',base:36,max:150,ring:false,desc:'Multi-sport facility named after the legendary sprinter Milkha Singh — with cricket ground, athletics track, badminton and volleyball courts. Adjacent Hospital ensures immediate medical care.',feats:['Cricket Ground','Athletics Track','Hospital']},
];
...
function onHeroScroll(){
  ...
  const slotF=progress*9,slotIdx=Math.floor(slotF),slotT=slotF-slotIdx;
...
/* ── Show register panel ── */
function cusbShowRegister() {
  var panels = document.querySelectorAll('.lc-panel');
  var tabs   = document.querySelectorAll('.lc-tab');
  panels.forEach(function(p){ p.classList.remove('active'); });
  tabs.forEach(function(t){ t.classList.remove('active'); });
  var rp = document.getElementById('panel-register');
  if(rp) rp.classList.add('active');
}

/* ── Register new student ── */
function cusbDoRegister() {
  var errEl = document.getElementById('reg-err');
  if(errEl) errEl.style.display = 'none';

  var enroll = (document.getElementById('reg-enroll')||{}).value||'';
  var name   = (document.getElementById('reg-name')||{}).value||'';
  var email  = (document.getElementById('reg-email')||{}).value||'';
  var pass   = (document.getElementById('reg-pass')||{}).value||'';
  var prog   = (document.getElementById('reg-prog')||{}).value||'';
  var sem    = (document.getElementById('reg-sem')||{}).value||'';

  if(!enroll.trim() || !name.trim() || !email.trim() || !pass.trim()) {
    if(errEl) { errEl.textContent = '⚠ Enrollment ID, name, email and password are required.'; errEl.style.display = 'block'; }
    return;
  }
  ...
  if(msgEl) msgEl.textContent = 'Creating your account…';

  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || 'https://cusb-website.onrender.com/api';

  fetch(apiBase + '/portal/register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: enroll.trim(), password: pass.trim(), name: name.trim(), email: email.trim(), role: 'student', prog: prog.trim(), sem: sem.trim() })
  })
  ...
  .then(function(data){
    ...
    var user = data.user;
    try { sessionStorage.setItem('cusbPortalToken', data.token); sessionStorage.setItem('cusbPortalUser', JSON.stringify(user)); } catch(e){}
    _cusbUser = user;
    ...
    updateNavPill(user);
    setTimeout(function(){ cusbClosePortal(); showDashboard(user); }, 1400);
  })
  .catch(function(){
    if(loading) loading.style.display = 'none';
    if(tabsArea) tabsArea.style.display = 'block';
    cusbShowRegister();
    if(errEl) { errEl.textContent = '⚠ Could not reach server. Check connection and try again.'; errEl.style.display = 'block'; }
  });
}
...
function cusbDoLogin(role) {
  ...
  if(!inp.id.trim() || !inp.pass.trim()) {
    if(errEl) {
      errEl.textContent = '⚠ Please enter your enrollment ID and password.';
  ...
  fetch(apiBase + '/portal/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: inp.id.trim(), password: inp.pass.trim(), role: role })
  })
  .then(function(res) { return res.json(); })
  .then(function(data) {
    if(loading) loading.style.display = 'none';
    ...
    if(!data.success) {
      ...
      if(tabsArea) tabsArea.style.display = 'block';
      if(errEl) {
        errEl.textContent = '⚠ ' + (data.message || 'Invalid enrollment ID or password. Please try again.');
        errEl.style.display = 'block';
      }
      return;
    }

    /* ── Success: persist token + user ── */
    var user = data.user;
    try {
      sessionStorage.setItem('cusbPortalToken', data.token);
      sessionStorage.setItem('cusbPortalUser', JSON.stringify(user));
    } catch(e) {}
    ...
    var snEl = document.getElementById('lc-sname');
    var seEl = document.getElementById('lc-semail');
    if(snEl) snEl.textContent = user.name || user.username;
    if(seEl) seEl.textContent = user.email || '';
...
function updateNavPill(user) {
  ...
  /* Show Admin Panel link if role = admin */
  var adminLink = document.getElementById('navAdminPanelLink');
  if (adminLink) {
...
function showDashboard(user) {
  ...
  // Fetch live student profile, then build attendance, timetable, results, stats
  fetchStudentProfile(function(profile) {
    buildAttendance(profile);
    buildTimetable(profile);
    buildResults(profile);
    updateStudentDashStats(profile);
  });
  ...
  if(user.role === 'Faculty') {
    ...
    fetchAdminStats();
    document.querySelectorAll('.admin-only-nav').forEach(function(el){ el.style.display='flex'; });
  } else {
...
function cusbSignOut() {
  _cusbUser = null;
  try { sessionStorage.removeItem('cusbPortalUser'); sessionStorage.removeItem('cusbPortalToken'); } catch(e){}
  ...
  // Hide admin panel link on sign out
  var al = document.getElementById('navAdminPanelLink');
  if(al) al.style.display = 'none';
  document.querySelectorAll('.admin-only-nav').forEach(function(el){ el.style.display='none'; });
}

/* ── Student profile cache ── */
var _studentProfile = null;

/* ── Fetch student profile from API ── */
function fetchStudentProfile(cb) {
  var token = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  fetch(apiBase + '/portal/student-profile', {
    headers: { 'Authorization': 'Bearer ' + token }
  })
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(d.success){ _studentProfile = d.data; }
    if(cb) cb(d.success ? d.data : null);
  })
  .catch(function(){ if(cb) cb(null); });
}

/* ── Build attendance bars from profile ── */
function buildAttendance(profile) {
  var cont = document.getElementById('attBars');
  if(!cont) return;
  var subjects = (profile && profile.attendanceSubjects && profile.attendanceSubjects.length)
    ? profile.attendanceSubjects : [];
  if(!subjects.length){
    cont.innerHTML = '<div style="padding:16px;font-family:Arial,sans-serif;font-size:13px;color:rgba(44,36,24,0.5);text-align:center;">No attendance data available. Contact your administrator.</div>';
    return;
  }
  cont.innerHTML = subjects.map(function(s){
    var pct = s.total > 0 ? Math.round((s.attended / s.total) * 100) : 0;
    var cls = pct>=75?'ok': pct>=65?'warn':'low';
    return '<div class="att-row">'
      ...
      +'<div class="att-bar-wrap"><div class="att-bar '+cls+'" style="width:'+pct+'%"></div></div>'
      +'<div class="att-pct '+cls+'">'+pct+'%</div>'
      +'</div>';
  }).join('');
  var ta=subjects.reduce(function(a,s){return a+s.attended;},0);
  var tt=subjects.reduce(function(a,s){return a+s.total;},0);
  var ov=tt>0?Math.round(ta/tt*100):0;
  var b75=subjects.filter(function(s){return s.total>0&&Math.round(s.attended/s.total*100)<75;}).length;
  var ovEl=document.querySelector('#page-attendance .dash-stat.green .dash-stat-val');
  var clEl=document.querySelector('#page-attendance .dash-stat.blue .dash-stat-val');
  var bwEl=document.querySelector('#page-attendance .dash-stat.gold .dash-stat-val');
  if(ovEl) ovEl.textContent=ov+'%';
  if(clEl) clEl.textContent=ta+'/'+tt;
  if(bwEl) bwEl.textContent=b75;
}

/* ── Build timetable from profile ── */
function buildTimetable(profile) {
  var grid = document.getElementById('ttGrid');
  if(!grid) return;
  var days  = ['MON','TUE','WED','THU','FRI','SAT'];
  var times = (profile && profile.timeSlots && profile.timeSlots.length)
    ? profile.timeSlots : ['9\u201310 AM','10\u201311 AM','11\u201312 PM','2\u20133 PM','3\u20134 PM'];
  var cells = (profile && profile.timetableCells) ? profile.timetableCells : [];
  var todayCol = new Date().getDay();
  if(!cells.length){
    grid.innerHTML='<div style="grid-column:1/-1;padding:20px;font-family:Arial,sans-serif;font-size:13px;color:rgba(44,36,24,0.5);text-align:center;">Timetable not configured. Contact administrator.</div>';
    return;
  }
  var lookup={};
  cells.forEach(function(c){ if(!lookup[c.slotIndex])lookup[c.slotIndex]={}; lookup[c.slotIndex][c.day]=c; });
  ...
  times.forEach(function(t,ri){
    ...
    for(var ci=0;ci<6;ci++){
      ...
      var cell=lookup[ri]&&lookup[ri][ci];
      if(!cell||!cell.subject){html+='<div class="tt-cell empty '+td+'"></div>';continue;}
      html+='<div class="tt-cell '+(cell.colorKey||'c1')+' '+td+'">'+cell.subject+'</div>';
    }
  });
  grid.innerHTML=html;
}

/* ── Update student dashboard stats from profile ── */
function updateStudentDashStats(profile) {
  if(!profile) return;
  var subs=profile.attendanceSubjects||[];
  var ta=subs.reduce(function(a,s){return a+s.attended;},0);
  var tt=subs.reduce(function(a,s){return a+s.total;},0);
  var ov=tt>0?Math.round(ta/tt*100):null;
  var attEl=document.querySelector('#page-dashboard .dash-stat.blue .dash-stat-val');
  if(attEl&&ov!==null) attEl.textContent=ov+'%';
  var crEl=document.querySelector('#page-dashboard .dash-stat.green .dash-stat-val');
  if(crEl&&profile.creditsEarned) crEl.textContent=profile.creditsEarned;
  if(profile.examStartDate){
    var dEl=document.querySelector('#page-dashboard .dash-stat.gold .dash-stat-val');
    if(dEl){var diff=Math.ceil((new Date(profile.examStartDate)-new Date())/(86400000));dEl.textContent=diff>0?diff:'Today';}
  }
  var nEl=document.querySelector('#page-dashboard .dash-stat.purple .dash-stat-val');
  if(nEl&&profile.newNoticesCount!=null) nEl.textContent=profile.newNoticesCount;
}

/* ── Build results from profile ── */
function buildResults(profile) {
  if(!profile) return;
  var results=profile.semesterResults||[];
  if(!results.length) return;
  var latest=results[results.length-1];
  var summaryNums=document.querySelectorAll('#page-results .dash-card:first-child [style*="font-size:28px"]');
  if(summaryNums[0]&&profile.currentSgpa) summaryNums[0].childNodes[0].textContent=profile.currentSgpa+' ';
  if(summaryNums[1]&&profile.cgpa) summaryNums[1].childNodes[0].textContent=profile.cgpa+' ';
  if(summaryNums[2]&&latest.credits) summaryNums[2].childNodes[0].textContent=latest.credits+' ';
  var tbody=document.querySelector('#page-results .dash-card:nth-child(2) tbody');
  if(tbody&&latest.subjects&&latest.subjects.length){
    tbody.innerHTML=latest.subjects.map(function(s){
      var total=s.internal+s.external; var totalMax=s.internalMax+s.externalMax;
      var gc=s.grade==='O'?'grade-O':s.grade==='A+'?'grade-A':'grade-B';
      return '<tr><td>'+(s.courseCode||'\u2014')+'</td><td>'+s.subject+'</td>'
        +'<td>'+s.internal+'/'+s.internalMax+'</td><td>'+s.external+'/'+s.externalMax+'</td>'
        +'<td>'+total+'/'+totalMax+'</td><td><span class="grade-badge '+gc+'">'+(s.grade||'\u2014')+'</span></td>'
        +'<td>'+(s.points||'\u2014')+'</td></tr>';
    }).join('');
  }
  var prevTbody=document.querySelector('#page-results .dash-card:nth-child(3) tbody');
  if(prevTbody&&results.length){
    prevTbody.innerHTML=results.map(function(r){
      var cls=r.sgpa>=8.5?'grade-O':r.sgpa>=7.5?'grade-A':'grade-B';
      return '<tr><td>'+r.semesterLabel+'</td><td>'+r.sgpa+'</td><td>'+r.credits+'</td>'
        +'<td><span class="grade-badge '+cls+'">Pass</span></td></tr>';
    }).join('');
  }
}
...
function fetchAdminStats() {
  var token = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  fetch(apiBase + '/portal/admin/stats', { headers:{ 'Authorization':'Bearer '+token } })
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(!d.success) return;
    var st = d.data;
    var sel = document.getElementById('adminStatStudents');
    var pu  = document.getElementById('adminStatPortalUsers');
    var ps  = document.getElementById('adminStatPortalStudents');
    if(sel) sel.textContent = (st.totalStudents||0).toLocaleString();
    if(pu)  pu.textContent  = (st.totalPortalUsers||0).toLocaleString();
    if(ps)  ps.textContent  = (st.totalPortalStudents||0).toLocaleString();
  })
  .catch(function(){});
}
...
var _adminPUPage = 1;
var _adminPUSearch = '';
var _adminPUEditId = null;
var _adminPUProfileUserId = null;

function loadAdminPortalUsers(page, search) {
  _adminPUPage = page || 1;
  _adminPUSearch = (search !== undefined) ? search : _adminPUSearch;
  var token = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  var tbody = document.getElementById('adminPUBody');
  if(tbody) tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:20px;color:rgba(44,36,24,0.4);">Loading…</td></tr>';
  fetch(apiBase+'/portal/admin/portal-users?page='+_adminPUPage+'&limit=15&search='+encodeURIComponent(_adminPUSearch), {
    headers:{ 'Authorization':'Bearer '+token }
  })
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(!d.success){ return; }
    var tbody = document.getElementById('adminPUBody');
    var info  = document.getElementById('adminPUInfo');
    if(info) info.textContent = 'Total: '+d.total+' users';
    if(!tbody) return;
    if(!d.data.length){
      tbody.innerHTML='<tr><td colspan="6" style="text-align:center;padding:20px;color:rgba(44,36,24,0.4);">No users found.</td></tr>';
      return;
    }
    tbody.innerHTML = d.data.map(function(u){
      var roleCls = u.role==='admin'?'grade-O':u.role==='faculty'?'grade-A':'grade-B';
      return '<tr>'
        +'<td>'+u.username+'</td>'
        +'<td>'+u.name+'</td>'
        +'<td>'+u.email+'</td>'
        +'<td><span class="grade-badge '+roleCls+'">'+u.role+'</span></td>'
        +'<td>'+(u.prog||'—')+'</td>'
        +'<td style="white-space:nowrap;">'
          +'<button onclick="adminPUEditUser(\''+u._id+'\','+JSON.stringify(u).replace(/'/g,'\\\'')+')" style="padding:4px 10px;border-radius:8px;border:1px solid rgba(201,151,43,0.3);background:rgba(201,151,43,0.08);font-family:Arial,sans-serif;font-size:11px;cursor:pointer;margin-right:4px;">✏️ Edit</button>'
          +'<button onclick="adminPUManageProfile(\''+u._id+'\',\''+u.name+'\')" style="padding:4px 10px;border-radius:8px;border:1px solid rgba(66,133,244,0.3);background:rgba(66,133,244,0.08);font-family:Arial,sans-serif;font-size:11px;cursor:pointer;margin-right:4px;">📊 Data</button>'
          +'<button onclick="adminPUDeleteUser(\''+u._id+'\',\''+u.name+'\')" style="padding:4px 10px;border-radius:8px;border:1px solid rgba(234,67,53,0.3);background:rgba(234,67,53,0.08);font-family:Arial,sans-serif;font-size:11px;cursor:pointer;">🗑️</button>'
        +'</td>'
        +'</tr>';
    }).join('');
    // Pagination
    var pg = document.getElementById('adminPUPagination');
    if(pg){
      var btns='';
      if(_adminPUPage>1) btns+='<button onclick="loadAdminPortalUsers('+(_adminPUPage-1)+')" style="'+paginBtnStyle()+'">← Prev</button> ';
      btns+='<span style="font-family:Arial,sans-serif;font-size:12px;color:rgba(44,36,24,0.5);">Page '+_adminPUPage+' / '+d.pages+'</span> ';
      if(_adminPUPage<d.pages) btns+='<button onclick="loadAdminPortalUsers('+(_adminPUPage+1)+')" style="'+paginBtnStyle()+'">Next →</button>';
      pg.innerHTML=btns;
    }
  })
  .catch(function(){});
...
function paginBtnStyle(){ return 'padding:4px 12px;border-radius:8px;border:1px solid rgba(201,151,43,0.2);background:rgba(201,151,43,0.06);font-family:Arial,sans-serif;font-size:12px;cursor:pointer;'; }

function adminPUOpenAdd() {
  _adminPUEditId = null;
  document.getElementById('adminPUModalTitle').textContent = 'Add Portal User';
  ['adminPUf-username','adminPUf-password','adminPUf-name','adminPUf-email','adminPUf-prog','adminPUf-sem'].forEach(function(id){
    var el=document.getElementById(id); if(el) el.value='';
  });
  var roleEl = document.getElementById('adminPUf-role'); if(roleEl) roleEl.value='student';
  document.getElementById('adminPUf-password').placeholder='Password (required)';
  document.getElementById('adminPUModal').style.display='flex';
}

function adminPUEditUser(id, user) {
  _adminPUEditId = id;
  document.getElementById('adminPUModalTitle').textContent = 'Edit User';
  document.getElementById('adminPUf-username').value = user.username||'';
  document.getElementById('adminPUf-password').value = '';
  document.getElementById('adminPUf-password').placeholder = 'Leave blank to keep current';
  document.getElementById('adminPUf-name').value  = user.name||'';
  document.getElementById('adminPUf-email').value = user.email||'';
  document.getElementById('adminPUf-role').value  = user.role||'student';
  document.getElementById('adminPUf-prog').value  = user.prog||'';
  document.getElementById('adminPUf-sem').value   = user.sem||'';
  document.getElementById('adminPUModal').style.display='flex';
}

function adminPUSave() {
  var token   = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  var body = {
    username: document.getElementById('adminPUf-username').value.trim(),
    name:     document.getElementById('adminPUf-name').value.trim(),
    email:    document.getElementById('adminPUf-email').value.trim(),
    role:     document.getElementById('adminPUf-role').value,
    prog:     document.getElementById('adminPUf-prog').value.trim(),
    sem:      document.getElementById('adminPUf-sem').value.trim(),
  };
  var pw = document.getElementById('adminPUf-password').value;
  if(pw) body.password = pw;

  var url    = _adminPUEditId ? apiBase+'/portal/admin/portal-users/'+_adminPUEditId : apiBase+'/portal/admin/portal-users';
  var method = _adminPUEditId ? 'PUT' : 'POST';

  var btn = document.getElementById('adminPUSaveBtn'); btn.disabled=true; btn.textContent='Saving…';
  fetch(url, { method:method, headers:{ 'Content-Type':'application/json','Authorization':'Bearer '+token }, body:JSON.stringify(body) })
  .then(function(r){ return r.json(); })
  .then(function(d){
    btn.disabled=false; btn.textContent='Save';
    if(d.success){ document.getElementById('adminPUModal').style.display='none'; loadAdminPortalUsers(_adminPUPage); fetchAdminStats(); }
    else { alert(d.message||'Save failed'); }
  })
  .catch(function(){ btn.disabled=false; btn.textContent='Save'; alert('Network error'); });
}

function adminPUDeleteUser(id, name) {
  if(!confirm('Delete portal user "'+name+'"? This also deletes their attendance, results and timetable data.')) return;
  var token   = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  fetch(apiBase+'/portal/admin/portal-users/'+id, { method:'DELETE', headers:{ 'Authorization':'Bearer '+token }})
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(d.success){ loadAdminPortalUsers(_adminPUPage); fetchAdminStats(); }
    else alert(d.message||'Delete failed');
  });
}

/* ── Admin: manage student profile data (attendance/results/timetable) ── */
function adminPUManageProfile(userId, userName) {
  _adminPUProfileUserId = userId;
  document.getElementById('adminProfModalTitle').textContent = 'Student Data — '+userName;
  document.getElementById('adminProfModal').style.display='flex';
  adminProfLoad(userId);
}

function adminProfLoad(userId) {
  var token   = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  document.getElementById('adminProfStatus').textContent = 'Loading…';
  fetch(apiBase+'/portal/admin/student-profiles/'+userId, { headers:{ 'Authorization':'Bearer '+token }})
  .then(function(r){ return r.json(); })
  .then(function(d){
    document.getElementById('adminProfStatus').textContent = '';
    if(!d.success) return;
    var p = d.data.profile;
    document.getElementById('adminProfSemLabel').value      = p.currentSemesterLabel||'';
    document.getElementById('adminProfExamDate').value      = p.examStartDate ? p.examStartDate.split('T')[0] : '';
    document.getElementById('adminProfCredits').value       = p.creditsEarned||'';
    document.getElementById('adminProfNoticeCount').value   = p.newNoticesCount||0;
    document.getElementById('adminProfCgpa').value          = p.cgpa||'';
    document.getElementById('adminProfSgpa').value          = p.currentSgpa||'';
    // Attendance JSON
    document.getElementById('adminProfAttJSON').value       = JSON.stringify(p.attendanceSubjects||[], null, 2);
    // Timetable cells JSON
    document.getElementById('adminProfTTJSON').value        = JSON.stringify(p.timetableCells||[], null, 2);
    // Results JSON
    document.getElementById('adminProfResultJSON').value    = JSON.stringify(p.semesterResults||[], null, 2);
  });
}

function adminProfSave() {
  var token   = sessionStorage.getItem('cusbPortalToken') || '';
  var apiBase = (window.APP_CONFIG && window.APP_CONFIG.apiBase) || '/api';
  var body = {
    currentSemesterLabel: document.getElementById('adminProfSemLabel').value.trim(),
    creditsEarned:        parseInt(document.getElementById('adminProfCredits').value)||0,
    newNoticesCount:      parseInt(document.getElementById('adminProfNoticeCount').value)||0,
    cgpa:                 parseFloat(document.getElementById('adminProfCgpa').value)||0,
    currentSgpa:          parseFloat(document.getElementById('adminProfSgpa').value)||0,
  };
  var examDate = document.getElementById('adminProfExamDate').value;
  if(examDate) body.examStartDate = examDate;
  try { body.attendanceSubjects = JSON.parse(document.getElementById('adminProfAttJSON').value); } catch(e){ alert('Invalid attendance JSON: '+e.message); return; }
  try { body.timetableCells     = JSON.parse(document.getElementById('adminProfTTJSON').value);  } catch(e){ alert('Invalid timetable JSON: '+e.message); return; }
  try { body.semesterResults    = JSON.parse(document.getElementById('adminProfResultJSON').value); } catch(e){ alert('Invalid results JSON: '+e.message); return; }

  var btn = document.getElementById('adminProfSaveBtn'); btn.disabled=true; btn.textContent='Saving…';
  fetch(apiBase+'/portal/admin/student-profiles/'+_adminPUProfileUserId, {
  ...
  .then(function(d){
    ...
    if(d.success){ document.getElementById('adminProfStatus').textContent='✓ Saved successfully'; }
    else { alert(d.message||'Save failed'); }
  })
  .catch(function(){ btn.disabled=false; btn.textContent='Save Data'; alert('Network error'); });
}
